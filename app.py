...
def apply():
    if request.method == 'POST':
        try:
            ...
            # ২. ফাইল রিসিভ করা (request.files ব্যবহার করে)
            photo_file = request.files.get('photo')
            sig_file = request.files.get('signature')

            if not photo_file or not sig_file:
                flash("Both Photo and Signature are required!", "danger")
                return redirect(request.url)

            # ৩. ImgBB-তে ইমেজ আপলোড এবং লিঙ্ক গ্রহণ
            photo_url = upload_to_imgbb(photo_file)
            sig_url = upload_to_imgbb(sig_file)

            if not photo_url or not sig_url:
                flash("Image upload failed! Please try again.", "danger")
                return redirect(request.url)

            # ৪. রোল এবং রেজিস্ট্রেশন নম্বর জেনারেট করা
            # (নিশ্চিত করুন আপনার generate_numbers() ফাংশনটি ডিফাইন করা আছে)
            ...
            student_data = {
                ...
                "photo_url": photo_url,  # এখন থেকে লিভ টাইম লিঙ্ক সেভ হবে
