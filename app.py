...
def apply():
    if request.method == 'POST':
        try:
            ...
            # ২. ফাইল রিসিভ ও ভ্যালিডেশন
            photo = request.files.get('photo')
            sig = request.files.get('signature')

            if not photo or photo.filename == '' or not sig or sig.filename == '':
                flash("Photo and Signature are required!", "danger")
                return redirect(request.url)

            # ৩. রোল ও রেজিস্ট্রেশন নম্বর জেনারেট
    ...
    # --- এটিই সেন্টার লিস্ট সমস্যার সমাধান ---
    # ডাটাবেসের 'centers' কালেকশন থেকে ডাটা নিয়ে আসা
    all_centers = list(mongo.db.centers.find().sort("center_code", 1))
    return render_template("apply.html", centers=all_centers)
