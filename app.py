...
def apply():
    if request.method == 'POST':
        try:
            ...
            # 2. Get Compressed Base64 Images from Hidden Inputs
            photo_data = request.form.get('compressed_photo')
            sig_data = request.form.get('compressed_sig')

            if not photo_data or not sig_data:
                flash("Photo and Signature are required!", "danger")
                return redirect(request.url)

            # 3. Generate Roll & Registration
            reg_no, roll_no = generate_numbers()

            # 4. Save Base64 Images to Filesystem
            upload_path = app.config.get('UPLOAD_FOLDER', 'static/uploads')
            if not os.path.exists(upload_path):
                os.makedirs(upload_path)

            def save_b64(b64_str, prefix, roll):
                # Clean header: 'data:image/jpeg;base64,/9j/...' -> '/9j/...'
                header, encoded = b64_str.split(",", 1)
                img_data = base64.b64decode(encoded)
                filename = f"{prefix}_{roll}.jpg"
                filepath = os.path.join(upload_path, filename)
                with open(filepath, "wb") as f:
                    f.write(img_data)
                return filename

            p_name = save_b64(photo_data, "photo", roll_no)
            s_name = save_b64(sig_data, "sig", roll_no)

            # 5. Build Database Object (Including Gender and Bengali Fields)
            student_data = {
                "roll_no": str(roll_no),
                "reg_no": str(reg_no),
            ...
            flash("An error occurred. Please ensure all fields are correct.", "danger")
            return redirect(request.url)

    # GET Method: Fetch Centers and Institutes for dropdowns/search
...
def serial_allocation():
    ...
    if request.method == 'POST':
        try:
            ...
            school_filter = request.form.get('institution') 
            start_roll = int(request.form.get('start_roll'))
            start_reg = int(request.form.get('start_reg'))

            # কোয়েরি ফিল্টার
            query = {"student_class": target_class, "status": "Verified"}
            
            # যদি ড্রপডাউন থেকে নির্দিষ্ট স্কুল সিলেক্ট করা হয়
            if school_filter:
                query["institute_en"] = school_filter

            # স্টুডেন্টদের নামের ক্রমানুসারে সর্ট করে নিয়ে আসা
            students = list(mongo.db.students.find(query).sort("name_en", 1))

            if not students:
                flash("নির্বাচিত ক্রাইটেরিয়া অনুযায়ী কোনো ছাত্র পাওয়া যায়নি।", "warning")
                return redirect(request.url)
            ...
            flash(f"সফলভাবে {len(students)} জন ছাত্রের সিরিয়াল আপডেট হয়েছে।", "success")
            
        except Exception as e:
            flash(f"Error: {str(e)}", "danger")

    # প্রতিষ্ঠানের কালেকশন থেকে সব নাম নিয়ে আসা
    # আপনার ডাটাবেসে ফিল্ডের নাম 'name' বা 'institute_name' হতে পারে, সেটি চেক করে নিন
    inst_docs = list(mongo.db.institutions.find({}, {"name": 1, "institute_en": 1, "_id": 0}))
    
    # ড্রপডাউনের জন্য নামের লিস্ট তৈরি (যদি ফিল্ডের নাম institute_en হয়)
    institutions_list = []
    for doc in inst_docs:
        name = doc.get('institute_en') or doc.get('name')
        if name:
            institutions_list.append(name)
    
    institutions_list = sorted(institutions_list)

    return render_template('admin_serial.html', institutions=institutions_list)
